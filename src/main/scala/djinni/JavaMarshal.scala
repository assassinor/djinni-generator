...
class JavaMarshal(spec: Spec) extends Marshal(spec) {
  ...
  override def toCpp(tm: MExpr, expr: String): String =
    throw new AssertionError("direct java to cpp conversion not possible")
  override def fromCpp(tm: MExpr, expr: String): String =
    throw new AssertionError("direct cpp to java conversion not possible")
  ...
  def nullityAnnotation(ty: Option[TypeRef]): Option[String] =
    ty.map(nullityAnnotation).getOrElse(None)
    ...
    ty.resolved.base match {
      ...
      case e: MExtern =>
        e.defType match {
          case DInterface => interfaceNullityAnnotation
          case DRecord =>
            if (e.java.reference.get) javaNonnullAnnotation else None
          case DEnum => javaNonnullAnnotation
        }
  ...
  private def toJavaType(tm: MExpr, packageName: Option[String]): String = {
    ...
    else tm.args.map(f(_, true)).mkString("<", ", ", ">")
  ...
  private def withPackage(packageName: Option[String], t: String) =
    packageName.fold(t)(_ + "." + t)
