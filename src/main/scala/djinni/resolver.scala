    ...
    try {
      ...
      for (typeDecl <- idl) {
        ...
        topScope = topScope.updated(
          ...
          typeDecl match {
            ...
            case td: ExternTypeDecl =>
              YamlGenerator.metaFromYaml(
                td,
      ...
      for (typeDecl <- idl) {
        ...
        for (typeParam <- typeDecl.params) {
          ...
          scope =
            scope.updated(typeParam.ident.name, MParam(typeParam.ident.name))
        }
      ...
      resolvedConsts.map(c => {
        if (c.ident.name == ref.name) {
          if (c.ty.resolved == ty)
            ...
            throw Error(
              ref.loc,
              s"Const ${ref.name} does not match type"
            ).toException
        }
      })
      ...
      case t: MPrimitive =>
        t.idlName match {
          ...
          case "i8" =>
            ...
            assert(
              value.asInstanceOf[Long].toByte == value,
              "Const value not a valid i8"
            )
          case "i16" =>
            assert(value.isInstanceOf[Long], "Const type mismatch: i16")
            assert(
              value.asInstanceOf[Long].toShort == value,
              "Const value not a valid i16"
            )
          case "i32" =>
            assert(value.isInstanceOf[Long], "Const type mismatch: i32")
            assert(
              value.asInstanceOf[Long].toInt == value,
              "Const value not a valid i32"
            )
          case "i64" =>
        ...
        d.defType match {
              ...
              map.get(field.ident.name) match {
                ...
                case None =>
                  throw new AssertionError(
                    s"Field ${field.ident.name} does not exist in const value"
                  )
              }
            }
          ...
          case DEnum => {
            ...
            if (!options.contains(opt.name))
              throw new AssertionError(
                s"Const type mismatch: enum ${d.name} does not have option ${opt.name}"
              )
          }
        }
      case _: MExtern =>
    ...
    for (f <- r.fields) {
      ...
      if (r.ext.any())
        if (r.derivingTypes.contains(DerivingType.Ord)) {
          throw new Error(
            f.ident.loc,
            "Cannot safely implement Ord on a record that may be extended"
          ).toException
        } else if (r.derivingTypes.contains(DerivingType.Eq)) {
          throw new Error(
            f.ident.loc,
            "Cannot safely implement Eq on a record that may be extended"
          ).toException
        }
      f.ty.resolved.base match {
        case MBinary | MList | MSet | MMap =>
          if (r.derivingTypes.contains(DerivingType.Ord))
            throw new Error(
              f.ident.loc,
              "Cannot compare collections in Ord deriving (Java limitation)"
            ).toException
        case MString =>
        case MDate   =>
        case MOptional =>
          if (r.derivingTypes.contains(DerivingType.Ord))
            throw new Error(
              f.ident.loc,
              "Cannot compare optional in Ord deriving"
            ).toException
        case t: MPrimitive =>
          t.idlName match {
            case "bool" =>
              if (r.derivingTypes.contains(DerivingType.Ord))
                throw new Error(
                  f.ident.loc,
                  "Cannot compare booleans in Ord deriving"
                ).toException
            case _ =>
          }
        case df: MDef =>
          df.defType match {
            ...
            case DRecord =>
              ...
              if (!r.derivingTypes.subsetOf(record.derivingTypes))
                throw new Error(
                  f.ident.loc,
                  s"Some deriving required is not implemented in record ${f.ident.name}"
                ).toException
            case DEnum =>
          }
        case e: MExtern =>
    ...
    scope.get(e.ident.name) match {
      case Some(meta) => {
        if (meta.numParams != e.args.length) {
          throw Error(
            e.ident.loc,
            "incorrect number of arguments to type \"" + e.ident.name
              + "\"; expecting " + meta.numParams + ", got " + e.args.length
          ).toException
        }
        ...
        if (meta == MOptional && margs.head.base == MOptional) {
          ...
          throw Error(
            e.ident.loc,
            "directly nested optionals not allowed"
          ).toException
        }
        MExpr(meta, margs)
      }
    ...
    def check(ident: Ident): Unit = {
      names.put(ident.name, ident.loc) match {
        case Some(existing) =>
          throw Error(
            ident.loc,
            "duplicate " + kind + " \"" + ident.name + "\" (previous definition: " + existing + ")"
          ).toException
        case _ =>
      }
    }
