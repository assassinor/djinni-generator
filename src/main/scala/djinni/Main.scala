...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var javaClassAccessModifier: JavaAccessModifier.Value =
    ...
    val argParser: OptionParser[Unit] = new scopt.OptionParser[Unit]("djinni") {
      ...
      def identStyle(
          optionName: String,
          defaultValue: String,
          update: IdentConverter => Unit
      ): OptionDef[String, Unit] = {
        opt[String](optionName)
          .valueName("...")
          .text(s"(default: $defaultValue)")
          .foreach(spec =>
      ...
      opt[String]("idl-include-path")
        .valueName("<path> ...")
        .optional()
        .unbounded()
        .foreach(x => idlIncludePaths = idlIncludePaths :+ x)
        .text(
      ...
      opt[File]("jni-out")
        .valueName("<out-folder>")
        .foreach(x => jniOutFolder = Some(x))
        .text(
          "The folder for the JNI C++ output files (Generator disabled if unspecified)."
        )
      opt[File]("jni-header-out")
        .valueName("<out-folder>")
        .foreach(x => jniHeaderOutFolderOptional = Some(x))
        .text(
          "The folder for the JNI C++ header files (default: the same as --jni-out)."
        )
      opt[String]("jni-include-prefix")
        .valueName("<prefix>")
        .foreach(jniIncludePrefix = _)
        .text(
          "The prefix for #includes of JNI header files from JNI C++ files."
        )
      opt[String]("jni-include-cpp-prefix")
        .valueName("<prefix>")
        .foreach(jniIncludeCppPrefix = _)
        .text(
      ...
      opt[File]("objc-out")
        .valueName("<out-folder>")
        .foreach(x => objcOutFolder = Some(x))
        .text(
          "The output folder for Objective-C files (Generator disabled if unspecified)."
        )
      opt[File]("objc-header-out")
        .valueName("<out-folder>")
        .foreach(x => objcHeaderOutFolderOptional = Some(x))
        .text(
          "The folder for the Objective-C header files (default: the same as --objc-out)."
        )
      opt[String]("objc-h-ext")
        .valueName("<ext>")
        .foreach(objcHeaderExt = _)
        .text(
          "The filename extension for Objective-C[++] header files (default: \"h\")"
        )
      opt[String]("objc-type-prefix")
        .valueName("<pre>")
        .foreach(objcTypePrefix = _)
        .text(
          "The prefix for Objective-C data types (usually two or three letters)"
        )
      opt[String]("objc-include-prefix")
        .valueName("<prefix>")
        .foreach(objcIncludePrefix = _)
        .text("The prefix for #import of header files from Objective-C files.")
      opt[String]("objc-swift-bridging-header")
        .valueName("<name>")
        .foreach(x => objcSwiftBridgingHeaderName = Some(x))
        .text(
          "The name of Objective-C Bridging Header used in XCode's Swift projects. The output folder is --objc-header-out."
        )
      opt[Boolean]("objc-closed-enums")
        .valueName("<true/false>")
        .foreach(x => objcClosedEnums = x)
        .text(
      ...
      opt[File]("objcpp-out")
        .valueName("<out-folder>")
        .foreach(x => objcppOutFolder = Some(x))
        .text(
          "The output folder for private Objective-C++ files (Generator disabled if unspecified)."
        )
      opt[File]("objcpp-header-out")
        .valueName("<out-folder>")
        .foreach(x => objcppHeaderOutFolderOptional = Some(x))
        .text(
          "The folder for the Objective-C++ header files (default: the same as --objcpp-out)."
        )
      opt[String]("objcpp-ext")
        .valueName("<ext>")
        .foreach(objcppExt = _)
        .text(
          "The filename extension for Objective-C++ files (default: \"mm\")"
        )
      opt[String]("objcpp-include-prefix")
        .valueName("<prefix>")
        .foreach(objcppIncludePrefix = _)
        .text(
          "The prefix for #import of Objective-C++ header files from Objective-C++ files."
        )
      opt[String]("objcpp-include-cpp-prefix")
        .valueName("<prefix>")
        .foreach(objcppIncludeCppPrefix = _)
        .text(
          "The prefix for #include of the main C++ header files from Objective-C++ files."
        )
      opt[String]("objcpp-include-objc-prefix")
        .valueName("<prefix>")
      ...
      opt[String]("cpp-extended-record-include-prefix")
        .valueName("<prefix>")
        .foreach(cppExtendedRecordIncludePrefix = _)
        .text(
          "The prefix path for #include of the extended record C++ header (.hpp) files"
        )
      opt[String]("objc-extended-record-include-prefix")
        .valueName("<prefix>")
        .foreach(objcExtendedRecordIncludePrefix = _)
        .text(
          "The prefix path for #import of the extended record Objective-C header (.h) files"
        )
      opt[String]("objcpp-namespace")
        .valueName("<prefix>")
      ...
      opt[File]("list-in-files")
        ...
        .foreach(x => inFileListPath = Some(x))
        .text("Optional file in which to write the list of input files parsed.")
      opt[File]("list-out-files")
        .valueName("<list-out-files>")
        .foreach(x => outFileListPath = Some(x))
        .text(
          "Optional file in which to write the list of output files produced."
        )
      opt[Boolean]("skip-generation")
        .valueName("<true/false>")
      ...
      identStyle(
        "ident-cpp-enum",
        "FOO_BAR",
        c => { cppIdentStyle = cppIdentStyle.copy(enum = c) }
      )
      identStyle(
        "ident-cpp-field",
        "foo_bar",
        c => { cppIdentStyle = cppIdentStyle.copy(field = c) }
      )
      identStyle(
        "ident-cpp-method",
        "foo_bar",
        c => { cppIdentStyle = cppIdentStyle.copy(method = c) }
      )
      identStyle(
        "ident-cpp-type",
        "FooBar",
        c => { cppIdentStyle = cppIdentStyle.copy(ty = c) }
      )
      identStyle(
        "ident-cpp-enum-type",
        "FooBar",
        c => { cppTypeEnumIdentStyle = c }
      )
      identStyle(
        "ident-cpp-type-param",
        "FooBar",
        c => { cppIdentStyle = cppIdentStyle.copy(typeParam = c) }
      )
      identStyle(
        "ident-cpp-local",
        "foo_bar",
        c => { cppIdentStyle = cppIdentStyle.copy(local = c) }
      )
      ...
      identStyle(
        "ident-java-enum",
        "FOO_BAR",
        c => { javaIdentStyle = javaIdentStyle.copy(enum = c) }
      )
      identStyle(
        "ident-java-field",
        "fooBar",
        c => { javaIdentStyle = javaIdentStyle.copy(field = c) }
      )
      identStyle(
        "ident-java-type",
        "FooBar",
        c => { javaIdentStyle = javaIdentStyle.copy(ty = c) }
      )
      identStyle(
        "ident-jni-class",
        "FooBar",
        c => { jniClassIdentStyleOptional = Some(c) }
      )
      identStyle(
        "ident-jni-file",
        "foo_bar",
      ...
      identStyle(
        "ident-objc-enum",
        "FooBar",
        c => { objcIdentStyle = objcIdentStyle.copy(enum = c) }
      )
      identStyle(
        "ident-objc-field",
        "fooBar",
        c => { objcIdentStyle = objcIdentStyle.copy(field = c) }
      )
      identStyle(
        "ident-objc-method",
        "fooBar",
        c => { objcIdentStyle = objcIdentStyle.copy(method = c) }
      )
      identStyle(
        "ident-objc-type",
        "FooBar",
        c => { objcIdentStyle = objcIdentStyle.copy(ty = c) }
      )
      identStyle(
        "ident-objc-type-param",
        "FooBar",
        c => { objcIdentStyle = objcIdentStyle.copy(typeParam = c) }
      )
      identStyle(
        "ident-objc-local",
        "fooBar",
        c => { objcIdentStyle = objcIdentStyle.copy(local = c) }
      )
      identStyle(
        "ident-objc-file",
        "FooBar",
    ...
    val cppHeaderOutFolder =
      if (cppHeaderOutFolderOptional.isDefined) cppHeaderOutFolderOptional
      else cppOutFolder
    ...
    val objcppIncludeObjcPrefix =
      objcppIncludeObjcPrefixOptional.getOrElse(objcppIncludePrefix)

    // Add ObjC prefix to identstyle
    objcIdentStyle = objcIdentStyle.copy(ty =
      IdentStyle.prefix(objcTypePrefix, objcIdentStyle.ty)
    )
    ...
    if (objcOutFolder.isDefined) {
      if (cppNamespace.isEmpty() && objcTypePrefix.isEmpty()) {
        System.err.println(
          "Error: At least one of [--cpp-namespace, --objc-type-prefix] needs to be set when generating Objective-C code."
        )
        System.exit(1); return
      }
    }
    ...
    val objcSwiftBridgingHeaderWriter =
      if (
        objcSwiftBridgingHeaderName.isDefined && objcOutFolder.isDefined && !skipGeneration
      ) {
        val objcSwiftBridgingHeaderFile = new File(
          objcHeaderOutFolder.get.getPath,
          objcSwiftBridgingHeaderName.get + ".h"
        )
        if (objcSwiftBridgingHeaderFile.getParentFile != null)
          createFolder(
            "output file list",
            objcSwiftBridgingHeaderFile.getParentFile
          )
